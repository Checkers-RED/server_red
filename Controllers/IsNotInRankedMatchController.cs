...
namespace server_red.Controllers
{
    [Route("[controller]")]
    [ApiController]
    public class IsNotInRankedMatchController : Controller
    {
        class output
        {
        ...
        public IsNotInRankedMatchController(IRedRepository db)
        {
            _db = db;
        }

        [HttpPost(Name = "IsNotInRankedMatch")]
        public IActionResult Get([FromBody] dynamic data)
        {
            try
            {
                ...
                if (curs != null)
                {
                    int inMatch = _db.IsNotInRankedMatch(curs.current_session!);
                    if (inMatch == 2)
                    {
                        output res = new()
                        {
